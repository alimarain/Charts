...
class _AnalyticsScreenState extends ConsumerState<AnalyticsScreen> {
  ...
  Future<void> _handleGlobalExport(String type, BuildContext context) async {
    ...
    if (filteredResult == null || !filteredResult.hasData) {
      scaffold.showSnackBar(const SnackBar(content: Text('No data available to export.')));
      return;
    }

    try {
      if (type == 'pdf') {
        scaffold.showSnackBar(const SnackBar(content: Text('Generating executive PDF report...')));
        await AnalyticsExportService.exportAndSharePdf(result: filteredResult, filter: filter);
      } else if (type == 'csv') {
        scaffold.showSnackBar(const SnackBar(content: Text('Exporting analytics CSV data...')));
        await AnalyticsExportService.exportAndShareCsv(result: filteredResult, filter: filter);
      }
    } catch (e) {
                    ...
                    AppHeader(
                      ...
                      actions: [
                        PopupMenuButton<String>(
                          icon: const Icon(Icons.ios_share_rounded, size: 18, color: Color(0xFF6B7280)),
                      ...
                      child: Builder(
                        builder: (context) {
                          if (analyticsState.isLoading && analyticsState.data == null) {
                            return const Center(
                              child: CircularProgressIndicator(color: Color(0xFF1B1638), strokeWidth: 2.5),
                            );
                          }
                              ...
                              child: Padding(
                                ...
                                child: Column(
                                  ...
                                  children: [
                                    ...
                                    Text('Could not load telemetry: ${analyticsState.errorMessage}', style: const TextStyle(fontSize: 12)),
                          ...
                          if (filteredResult == null || !filteredResult.hasData) {
                            return Center(
                              child: Column(
                                mainAxisSize: MainAxisSize.min,
                                children: [
                                  const Icon(Icons.filter_alt_off_rounded, size: 40, color: Color(0xFF9CA3AF)),
                                  const SizedBox(height: 10),
                          ...
                          return SingleChildScrollView(
                            ...
                            padding: EdgeInsets.fromLTRB(isDesktop ? 28 : 16, 20, isDesktop ? 28 : 16, 32),
                            child: Center(
